      ...
      <main className="flex-1 flex flex-col overflow-hidden">
              ...
              <Card key={player.id} className="border-none shadow-md rounded-2xl bg-white">
                <CardContent className="p-6">
                  ...
                  <div className="flex items-center justify-between py-3 border-t">
                    <div><p className="text-[10px] text-muted-foreground uppercase font-bold">Konto</p><p className={cn("text-lg font-bold", player.balance < 0 ? 'text-destructive' : 'text-emerald-600')}>{player.balance.toFixed(2)} €</p></div>
                    <Button size="icon" variant="ghost" onClick={() => { setEditingPlayer(player); setEditName(player.name); setEditEmail(player.email); setEditRole(player.role); setIsEditOpen(true); }}><ChevronRight className="h-4 w-4" /></Button>
                  </div>
                </CardContent>
              </Card>
            ))}
        ...
        <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
          <DialogContent>
            ...
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-4 items-center gap-4"><Label className="text-right">Name</Label><Input value={editName} onChange={e => setEditName(e.target.value)} className="col-span-3" /></div>
              <div className="grid grid-cols-4 items-center gap-4"><Label className="text-right">Rolle</Label>
